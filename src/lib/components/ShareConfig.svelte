<script lang="ts">
  ...
  let { owner, slug }: Props = $props();
  let url = $derived(`https://dotfyle.com/${owner}/${slug}`);
  let tweetText = $derived(`Neovim config by ${owner}`);
  let tweetUrl = $derived(
    `https://twitter.com/intent/tweet?url=${url}&text=${tweetText}&via=codicocodes&related=neovim`
  );
...
<button onclick={() => (open = !open)} class=" bg-black/30 p-2 rounded">
...
{#if open}
  ...
  >
    ...
    <div
      ...
      class="z-50 fixed bg-black/30 p-8 flex flex-col gap-8 rounded w-full mx-2 sm:w-1/3"
    >
