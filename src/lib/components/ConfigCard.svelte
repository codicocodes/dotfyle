<script lang="ts">
  ...
  let { avatar, owner, repo, stars, tools, screenshot }: Props = $props();
</script>
...
<div class="flex bg-white/10 px-4 py-3 rounded my-4">
  <div class="flex">
    <div class="flex flex-col gap-4 w-full">
      ...
      <div class="flex gap-1 items-center flex-wrap">
        <span class="py-1 rounded-full text-xs flex gap-1 items-center font-semibold mr-2">
          <Fa icon={faStar} />
          {stars}
        </span>
        {#each tools.slice(0, 4) as tool}
          <span class="text-xs font-semibold font-semibold bg-white/20 px-2 py-0.5 rounded">
            {tool}
          </span>
        {/each}
      </div>
  ...
  <div class="flex flex-grow justify-end items-center">
    <div class="relative text-base-300 hover:text-white hover:cursor-pointer">
      <img
        class="flex flex-end rounded max-h-[50px] blur-[1px]"
        height="20px"
        src={screenshot}
        alt="{owner}/{repo} config screenshot"
      />
      <Fa
