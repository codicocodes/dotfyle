...
describe('GithubMediaParser.findMediaUrls()', () => {
	...
	it('matches absolute {username}/media/raw/{branch}/{repo} png', () => {
		...
		const output =
			'https://raw.githubusercontent.com/echasnovski/media/raw/main/mini.nvim/logo/logo_clue.png';
	...
	it('matches absolute {username}/{repo}/blob/{branch}/ png', () => {
		...
		const output =
			'https://raw.githubusercontent.com/xiantang/darcula-dark.nvim/main/img/preview.png';
	...
	it('matches relative markdown image in repository', () => {
		...
		const output =
			'https://raw.githubusercontent.com/FrenzyExists/aquarium-vim/develop/screenshot.png';
	...
	it('matches relative markdown image in folder', () => {
		...
		const output =
			'https://raw.githubusercontent.com/FrenzyExists/aquarium-vim/develop/banners/screenshot.png';
		const media = run(url, 'FrenzyExists', 'aquarium-vim', 'develop');
		expect(media[0]).toEqual(output);
	});
	...
	it('matches relative html image in repository', () => {
		...
		const output =
			'https://raw.githubusercontent.com/FrenzyExists/aquarium-vim/develop/screenshots/banner.png';
		const media = run(url, 'FrenzyExists', 'aquarium-vim', 'develop');
		expect(media[0]).toEqual(output);
	});
